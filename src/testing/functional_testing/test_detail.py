...
def test_product_detail_function(driver, test_case):
    ...
    try:
        ...
        if "thành công" in test_case.get('Mô tả', '').lower():
            ...
            safe_type(login.email_input(), email)
            safe_type(login.password_input(), password)

            safe_click(login.login_button)
        ...
        print(f"TC: {test_case.get('ID')} - {test_case.get('Mô tả')}: {actual} - {result}")
        if row_number:
